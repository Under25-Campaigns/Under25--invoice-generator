...
function words(n) {
    ...
    function c(x) {
        ...
        if (x < 100)
            return b[Math.floor(x / 10)] + ' ' + a[x % 10];

        if (x < 1000)
            return a[Math.floor(x / 100)] + ' Hundred ' + c(x % 100);

        if (x < 100000)
            return c(Math.floor(x / 1000)) + ' Thousand ' + c(x % 1000);

        if (x < 10000000)
            return c(Math.floor(x / 100000)) + ' Lakh ' + c(x % 100000);
...
form.addEventListener("submit", async e => {
    ...
    document.getElementById("billPan").textContent =
        "PAN: " + g("pan");
    ...
    if (sig) {
        document.getElementById("invoiceSignature").src =
            sig;
    }
